...
function ENT:Draw()
    ...
    if LocalPlayer():GetPos():DistToSqr(self:GetPos()) <= 360000 then
        ...
        cam.Start3D2D(pos, ang, 0.12)
            draw.SimpleTextOutlined(KASSA_LANG.Get("register_name"), 'RegisterFont', 0, -340, color_white, 1, 1, 2, color_black)
            ...
            if status == 1 then
                draw.SimpleTextOutlined(KASSA_LANG.Get("being_robbed"), 'RegisterFont', 0, -180, Color(255, 0, 0), 1, 1, 2, color_black)
            elseif status == 2 then
                draw.SimpleTextOutlined(KASSA_LANG.Get("just_robbed"), 'RegisterFont', 0, -180, Color(255, 150, 0), 1, 1, 2, color_black)
            end
        cam.End3D2D()
    end
end